...
# from ..synchronization_rules import _match_pan_on_x_axis, _match_zoom_on_x_axis, _match_zoom_on_x_axis
...
#
#
#         # Start decode the whole video
#         slc = handler.get_slice(0, None)
#
#         handler.dedocde(slc)
#         position_data = data[slc]
#         tot_time = position_data.shape[0]
#         time = np.linspace(0, data.index[min(slc.stop - 10, tot_time - 1)], tot_time)
#         positions = np.stack((time, position_data, np.zeros_like(data))).T
