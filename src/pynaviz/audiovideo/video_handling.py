...
class VideoHandler:
    ...
    def __init__(
        ...
        if self.video_path.suffix == ".mkv":
            # mkv time is rounded to 3 digits, at least in the example audiovideo
    ...
    def extract_keyframe_times_and_points(
        ...
        """
        Extract the indices and timestamps of keyframes from a audiovideo file.

        This function decodes the audiovideo while skipping non-keyframes, and records:
        - The index of each keyframe in the full audiovideo frame sequence
        ...
        video_path : str or pathlib.Path
            The path to the audiovideo file.
        stream_index:
    ...
    def close(self):
        ...
        try:
            ...
            print("VideoHandler failed to close the audiovideo stream.")
        finally:
    ...
    def __getitem__(self, idx: slice | int):
        """
        Get item for audiovideo frame.

        Gets one or more frames from a audiovideo.
    ...
    # context protocol
    # (with VideoHandler(path) as audiovideo ensure closing)
    def __enter__(self):
        return self
